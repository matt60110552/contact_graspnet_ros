...
class GraspnetNode():
    def __init__(self) -> None:
        ...
        self.z_range = rospy.get_param("~z_range", [0.15, 2])
    ...
    def handle_grasp(self, request):
        try:
            ...
            pc_full = np.array(list(point_cloud2.read_points(request.pc_full,
                                                             field_names=("x", "y", "z"),
                                                             skip_nans=True)))
            pc_target = np.array(list(point_cloud2.read_points(request.pc_target,
                                                               field_names=("x", "y", "z"),
                                                               skip_nans=True)))
            mode = request.mode

            return self.inference(image_rgb, image_depth, segmap, cam_K, segmap_id, pc_full, pc_target, mode)
        except CvBridgeError as e:
            rospy.logerr(e)

    def inference(self, rgb, depth, segmap, cam_K, segmap_id, pc_full=None, pc_target=None, mode=0):
        if mode == 0:
            # If mode is 0, then get the pointcloud from depth image
            begin = time.time()
            ...
            # If mode is 1, then get the pointcloud from depth image
            pc_segments = {1: pc_target} # Assign the pc_segments manually
            begin = time.time()
            rospy.loginfo('Generating Grasps...')
            pred_grasps_cam, scores, contact_pts, _ = self.grasp_estimator.predict_scene_grasps(
                self.sess, pc_full, pc_segments=pc_segments,
